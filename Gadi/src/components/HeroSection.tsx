...
export default function HeroSection({onAuthRequired}:{onAuthRequired:()=>void}) {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden">
      ...
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6}} className="mt-4 max-w-xl text-gray-300">
        From daily rides to heavy transport - all in one platform.
      </motion.p>

      <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.3 }} className="mt-8 flex gap-8 text-gray-300">
        <Bike size={30}/>
        <Car size={30}/>
        <Bus size={30}/>
        <Truck size={30}/>
